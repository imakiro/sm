...
class TestSR(unittest.TestCase):
    ...
    @mock.patch('util.SMException.__init__',
    ...
    def test_coalesceLeaf_size_the_same(self,
        ...
        mock_exception_init.assert_called_with("VDI {uuid} could"
                                               " not be coalesced"
                                               .format(uuid=vdi_uuid))
    ...
    @mock.patch('cleanup.SR._snapshotCoalesce', autospec=True,
                return_value=True)
    @mock.patch('util.SMException.__init__', autospec=True, return_value=None)
    @mock.patch('cleanup.Util.log')
    def test_coalesceLeaf_size_bigger(self, mock_log, mock_exceptionInit,
        ...
        mock_exceptionInit.assert_called_with("VDI {uuid} could"
                                              " not be coalesced"
                                              .format(uuid=vdi_uuid))
    ...
    @mock.patch('cleanup.Util.log')
    def test_findLeafCoalesceable_forbiden1(self, mock_log):
        sr_uuid = uuid4()
        sr = create_cleanup_sr(uuid=str(sr_uuid))
    ...
    @mock.patch('cleanup.Util.log')
    def test_findLeafCoalesceable_forbiden2(self, mock_log):
        sr_uuid = uuid4()
        sr = create_cleanup_sr(uuid=str(sr_uuid))
        sr.xapi.srRecord =\
            {"other_config":
    ...
    @mock.patch('cleanup.Util.log')
    def test_findLeafCoalesceable_forbiden3(self, mock_log):
        sr_uuid = uuid4()
        sr = create_cleanup_sr(uuid=str(sr_uuid))
    ...
    @mock.patch('cleanup.Util.log')
    def test_findLeafCoalesceable_forbiden4(self, mock_log):
        sr_uuid = uuid4()
        sr = create_cleanup_sr(uuid=str(sr_uuid))
    ...
    @mock.patch('cleanup.Util.log')
    def test_findLeafCoalesceable_forbiden5(self, mock_log):
        sr_uuid = uuid4()
        sr = create_cleanup_sr(uuid=str(sr_uuid))
        sr.xapi.srRecord = {"other_config": {cleanup.VDI.DB_LEAFCLSC:
...
# Utils for testing gatherCandidates.
    ...
    def findLeafCoalesceable(self, mock_gatherCandidates, goodSize,
        ...
        mock_gatherCandidates.side_effect = fakeCandidates
        res = sr.findLeafCoalesceable()
        if expectedNothing:
    ...
    @mock.patch('cleanup.SR.gatherCandidates', autospec=True)
    def test_insufficient_space(self, mock_gatherCandidates, mock_getFreeSpace,
                                mock_leafCoalesceForbidden):
        """Good vdi calculates space less than remaining on sr"""
        self.findLeafCoalesceable(mock_gatherCandidates, 4)

    @mock.patch('cleanup.SR.leafCoalesceForbidden', autospec=True,
                return_value=False)
    @mock.patch('cleanup.SR.getFreeSpace', autospec=True, return_value=1024)
    @mock.patch('cleanup.SR.gatherCandidates', autospec=True)
    def test_space_equal(self, mock_gatherCandidates,
                         mock_getFreeSpace,
                         mock_leafCoalesceForbidden):
        """Good has calculates space equal to remaining space"""
        self.findLeafCoalesceable(mock_gatherCandidates, 1024)

    @mock.patch('cleanup.SR.leafCoalesceForbidden', autospec=True,
                return_value=False)
    @mock.patch('cleanup.SR.getFreeSpace', autospec=True, return_value=1024)
    @mock.patch('cleanup.SR.gatherCandidates', autospec=True)
    def test_fall_back_to_leaf_coalescing(self, mock_gatherCandidates,
                                          mock_getFreeSpace,
                                          mock_leafCoalesceForbidden):
        """Good VDI can can live coalesce and has right size"""
        self.findLeafCoalesceable(mock_gatherCandidates, 4096,
    ...
    @mock.patch('cleanup.SR.getFreeSpace', autospec=True, return_value=1024)
    @mock.patch('cleanup.SR.gatherCandidates', autospec=True)
    def test_leaf_coalescing_but_cannt_live_coalesce(self,
                                                     mock_gatherCandidates,
                                                     mock_getFreeSpace,
                                                     mock_leafCoalesceForbidden
                                                     ):
        """1st VDI is too big for snap but right size for live
        2nd VDI is too big for snap and too big for live"""
        vdi1, vdi2 = self.findLeafCoalesceable(mock_gatherCandidates, 4097,
